...
int main(int argc, char **argv) {
  ...
  static paramdef_t options[] = {
      ...
      {"snr",
       "Starting SNR, runs from SNR to SNR+%.1fdB in steps of %.1fdB. If n_frames is 1 then just SNR is simulated and "
       "MATLAB/OCTAVE output is generated",
       .dblptr = &snr0,
       .defdblval = -2.0,
       TYPE_DOUBLE,
       0},
      ...
      {"gchannel",
       "[A:M] Use 3GPP 25.814 SCM-A/B/C/D('A','B','C','D') or 36-101 EPA('E'), EVA ('F'),ETU('G') models (ignores delay spread and "
       "Ricean factor), Rayghleigh8 ('H'), Rayleigh1('I'), Rayleigh1_corr('J'), Rayleigh1_anticorr ('K'),  Rice8('L'), Rice1('M')",
       0,
       .strptr = NULL,
       .defstrval = NULL,
       TYPE_STRING,
  ...
  while ((res = getopt_long_only(argc, argv, "-", long_options, &option_index)) >= 0) {
    ...
    /* with these opstring and long_options getopt returns 1 for non-option arguments and '?' for unrecognized long options, refer
     * to 'man 3 getopt' */
    if (res == 1 || res == '?')
      continue;
