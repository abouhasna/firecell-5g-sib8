...
//------------------------------------------------------------------------------
{
  ...
  /* mandatory */
  {
    ...
    free(UEfirstReq->nas_pdu.buffer);
  }
...
//------------------------------------------------------------------------------
int ngap_gNB_initial_ctxt_fail(instance_t instance, ngap_initial_context_setup_fail_t *initial_ctxt_fail_p)
//------------------------------------------------------------------------------
{
  ...
  /* Retrieve the NGAP gNB instance associated with Mod_id */
  ngap_gNB_instance_p = ngap_gNB_get_instance(instance);
  DevAssert(initial_ctxt_fail_p != NULL);
  DevAssert(ngap_gNB_instance_p != NULL);

  if ((ue_context_p = ngap_get_ue_context(initial_ctxt_fail_p->gNB_ue_ngap_id)) == NULL) {
    /* The context for this gNB ue ngap id doesn't exist in the map of gNB UEs */
    NGAP_WARN("Failed to find ue context associated with gNB ue ngap id: 0x%08x\n", initial_ctxt_fail_p->gNB_ue_ngap_id);
    return -1;
  }
  /* Prepare the NGAP message to encode */
  memset(&pdu, 0, sizeof(pdu));
  ...
  head->value.present = NGAP_UnsuccessfulOutcome__value_PR_InitialContextSetupFailure;
  NGAP_InitialContextSetupFailure_t *out = &head->value.choice.InitialContextSetupFailure;
  /* mandatory */
  {
    asn1cSequenceAdd(out->protocolIEs.list, NGAP_InitialContextSetupFailureIEs_t, ie);
    ie->id = NGAP_ProtocolIE_ID_id_AMF_UE_NGAP_ID;
    ie->criticality = NGAP_Criticality_ignore;
  ...
  /* mandatory */
  {
    ...
    ie->value.choice.RAN_UE_NGAP_ID = initial_ctxt_fail_p->gNB_ue_ngap_id;
  }
   /* mandatory */
  {
    asn1cSequenceAdd(out->protocolIEs.list, NGAP_InitialContextSetupFailureIEs_t, ie);
    ...
    /*TO DO/Firecell: this value should be get from the initial_ctxt_fail_p structure*/
    ie->value.choice.Cause.present=NGAP_Cause_PR_radioNetwork;
    ie->value.choice.Cause.choice.radioNetwork=NGAP_CauseRadioNetwork_up_integrity_protection_not_possible;
  }
  /* optional */
  if (initial_ctxt_fail_p->nb_of_pdusessions_failed) {
    asn1cSequenceAdd(out->protocolIEs.list, NGAP_InitialContextSetupFailureIEs_t, ie);
    ...
    for (i = 0; i < initial_ctxt_fail_p->nb_of_pdusessions_failed; i++) {
      asn1cSequenceAdd(ie->value.choice.PDUSessionResourceFailedToSetupListCxtFail.list,
      ...
      /* pDUSessionID */
      item->pDUSessionID = initial_ctxt_fail_p->pdusessions_failed[i].pdusession_id;

      /* cause */
      switch (initial_ctxt_fail_p->pdusessions_failed[i].cause) {
        case NGAP_CAUSE_RADIO_NETWORK:
          pdusessionUnTransfer.cause.present = NGAP_Cause_PR_radioNetwork;
          pdusessionUnTransfer.cause.choice.radioNetwork = initial_ctxt_fail_p->pdusessions_failed[i].cause_value;
          break;

        case NGAP_CAUSE_TRANSPORT:
          pdusessionUnTransfer.cause.present = NGAP_Cause_PR_transport;
          pdusessionUnTransfer.cause.choice.transport = initial_ctxt_fail_p->pdusessions_failed[i].cause_value;
          break;

        case NGAP_CAUSE_NAS:
          pdusessionUnTransfer.cause.present = NGAP_Cause_PR_nas;
          pdusessionUnTransfer.cause.choice.nas = initial_ctxt_fail_p->pdusessions_failed[i].cause_value;
          break;

        case NGAP_CAUSE_PROTOCOL:
          pdusessionUnTransfer.cause.present = NGAP_Cause_PR_protocol;
          pdusessionUnTransfer.cause.choice.protocol = initial_ctxt_fail_p->pdusessions_failed[i].cause_value;
          break;
        case NGAP_CAUSE_NOTHING:
        case NGAP_CAUSE_MISC:
        default:
          pdusessionUnTransfer.cause.present = NGAP_Cause_PR_misc;
          pdusessionUnTransfer.cause.choice.misc = initial_ctxt_fail_p->pdusessions_failed[i].cause_value;
          break;
      }

      NGAP_INFO("initial context setup failure: failed pdusession ID %ld\n", item->pDUSessionID);
      ...
      /*TO DO/Firecell: Need to add this check :res.result.encoded should be more than 0*/
      if (res.buffer) {
        ...
        NGAP_WARN("Replaced AssertFatal for asn encoding failure for asn_DEF_NGAP_PDUSessionResourceSetupUnsuccessfulTransfer \n");
      }
