...
void ngap_gNB_handle_register_gNB(instance_t instance, ngap_register_gnb_req_t *ngap_register_gNB) {
  ...
  for (index = 0; index < ngap_register_gNB->nb_amf; index++) {
    ...
    /* TO DO: gNB restoration procedure is to proceed with existing AMF as gNB lost connection 
     * need to check why this condition was added initially
     * if (amf)
     *   continue;
     */
...
void ngap_gNB_handle_sctp_association_resp(instance_t instance, sctp_new_association_resp_t *sctp_new_association_resp) {
  ...
  if (sctp_new_association_resp->sctp_state != SCTP_STATE_ESTABLISHED) {
    NGAP_WARN("Received unsuccessful result for SCTP association (%u), instance %ld, cnx_id %u\n",
              ...
              instance,
              sctp_new_association_resp->ulp_cnx_id);
    ngap_handle_ng_setup_message(ngap_amf_data_p, sctp_new_association_resp->sctp_state);
    return;
  }
...
void *ngap_gNB_process_itti_msg(void *notUsed) {
  ...
  if (received_msg) {
    ...
    switch (ITTI_MSG_ID(received_msg)) {
      ...
      /**TBD */
      /** Need to define what basis this will trigger
       *  1. Spec says before the TNGrelocprep expires need trigger the HO Cancel. So in that case need to start NEW timer which
       *  should be less than "TNGrelocprep time value"
       * */
...
//-----------------------------------------------------------------------------
{
  ...
  /* optional */
  /* problem: at least OAI 5GC v1.5.1 (also later?) does not answer with NG
   * Setup Response if we send the below, so comment it for the moment.
