...
void ngap_handle_ng_setup_message(ngap_gNB_amf_data_t *amf_desc_p, int sctp_state) {
  if (sctp_state != SCTP_STATE_ESTABLISHED) {
    ...
    if (amf_desc_p->state == NGAP_GNB_STATE_CONNECTED) {
      ...
      NGAP_WARN("[SCTP state %u] Received ngap_handle_ng_setup_message\n", sctp_state);
...
static int ngap_gNB_handle_ng_setup_failure(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  ...
  // TODO: Need to handle better the NG_SETUP_FAIL in restoration procedure
  ngap_handle_ng_setup_message(amf_desc_p, SCTP_STATE_SHUTDOWN);
  return 0;
}
...
static int ngap_gNB_handle_ng_setup_response(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  ...
  amf_desc_p->ngap_gNB_instance->ngap_amf_associated_nb ++;
  ngap_handle_ng_setup_message(amf_desc_p, SCTP_STATE_ESTABLISHED);
...
static int ngap_gNB_handle_handover_preparation_failure(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  NGAP_INFO("HO LOG: Handover Preparation Failure is came to source gNB!\n");
  //  NGAP_AMF_UE_NGAP_ID_t       amf_ue_ngap_id;
  uint64_t                      amf_ue_ngap_id;
  NGAP_RAN_UE_NGAP_ID_t         ran_ue_ngap_id;
  ngap_gNB_amf_data_t          *amf_desc_p       = NULL;
   
  MessageDef *message_p = itti_alloc_new_message(TASK_NGAP, 0, NGAP_HANDOVER_PREPARATION_FAILURE);
  ngap_handover_preparation_fail_t *msg=&NGAP_HANDOVER_PREPARATION_FAILURE(message_p);
  memset(msg, 0, sizeof(*msg));

 NGAP_HandoverPreparationFailure_t     *container;
  ...
  /* id-AMF-UE-NGAP-ID */
  NGAP_FIND_PROTOCOLIE_BY_ID(NGAP_HandoverPreparationFailureIEs_t, ie, container, NGAP_ProtocolIE_ID_id_AMF_UE_NGAP_ID, true);
  asn_INTEGER2ulong(&(ie->value.choice.AMF_UE_NGAP_ID), &amf_ue_ngap_id);

  /* id-gNB-UE-NGAP-ID */
  NGAP_FIND_PROTOCOLIE_BY_ID(NGAP_HandoverPreparationFailureIEs_t, ie, container, NGAP_ProtocolIE_ID_id_RAN_UE_NGAP_ID, true);
  ran_ue_ngap_id = ie->value.choice.RAN_UE_NGAP_ID;

 ngap_gNB_ue_context_t *ue_desc_p = ngap_get_ue_context(ran_ue_ngap_id);
  if (!ue_desc_p) {
    NGAP_ERROR("[SCTP %u] Received HO prep failure Msg for non "
               "existing UE context 0x%06lx\n", assoc_id,
               ran_ue_ngap_id);
    return -1;
  }
...
const ngap_message_decoded_callback ngap_messages_callback[][3] = {
    ...
    {0, ngap_gNB_handle_handover_preparation_failure, 0}, /* HandoverPreparationFailure */
