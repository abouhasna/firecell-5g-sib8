...
int PRBalloc_to_locationandbandwidth0(int NPRB, int RBstart, int BWPsize)
{
  ...
  {
    LOG_E(NR_MAC, "PRBalloc_to_locationandbandwidth0 Illegal NPRB/RBstart Configuration (%d,%d) for BWPsize %d\n",
...
int fc_nb_ue_per_bwp(int bwp_size) {
  int nb_ue = 0;

  switch (bwp_size) {
    case 51:
      nb_ue = min(MAX_MOBILES_PER_GNB, 42);
      break;
    case 106:
    case 273:
    default:
      nb_ue = MAX_MOBILES_PER_GNB;
      break;
  }
  return nb_ue;
}
