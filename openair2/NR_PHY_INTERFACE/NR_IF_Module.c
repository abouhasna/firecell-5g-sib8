...
void handle_nr_ulsch(NR_UL_IND_t *UL_info)
{
  ...
  if (UL_info->rx_ind.number_of_pdus > 0 && UL_info->crc_ind.number_crcs > 0) {
    ...
    for (int i = 0; i < UL_info->rx_ind.number_of_pdus; i++) {
      ...
      nfapi_nr_crc_t *crc = &UL_info->crc_ind.crc_list[i];
      ...
      if(crc->rnti != rx->rnti)
      {
        /* Removed the assertion to avoid crash in case of mis-match between CRC RNTI and RX RNTI
         * Since the RNTI mis-match is possible setting the CRC status to 1 to indicate the MAC
         * to handle this reception is failed
         * */
         LOG_E(NR_MAC, "Replaced AssertFatal for mis-match between CRC RNTI %x and RX RNTI %x in handle_nr_ulsch\n",crc->rnti, rx->rnti);
         crc->tb_crc_status = 1;
      }
      LOG_D(NR_MAC,
