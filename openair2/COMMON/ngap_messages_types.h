...
#define NGAP_HANDOVER_PREPARATION_FAILURE(mSGpTR)           (mSGpTR)->ittiMsg.ngap_handover_preparation_fail
...
#define MAX_NUM_FIVEQI              26 // TODO find a better place
#define MAX_NUM_GBR_FLOWS           12
...
/*HO CANCEL */
typedef struct ngap_handover_cancel_s {
  uint32_t gNB_ue_ngap_id;
  uint64_t amf_ue_ngap_id;
  uint32_t cause; //5 for Handover DEsirable For Radio Reason
} ngap_handover_cancel_t;

/*HO PREPARATION FAILURE */
typedef struct ngap_handover_preparation_fail_s{
  uint32_t gNB_ue_ngap_id;
  uint64_t amf_ue_ngap_id;
  uint32_t cause; 
}ngap_handover_preparation_fail_t;

/*HO FAILURE */
typedef struct ngap_handover_failure_s{
  uint32_t gNB_ue_ngap_id;
  uint64_t amf_ue_ngap_id;
  uint32_t cause;
