    ...
    switch(typeandinfo->present) {
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib8:
        ...
        if(g_log->log_component[NR_RRC].level >= OAILOG_DEBUG)
          xer_fprint(stdout, &asn_DEF_NR_SIB8, (const void *) SI_info->sib8);
...
static void nr_rrc_handle_msg3_indication(NR_UE_RRC_INST_t *rrc, rnti_t rnti)
{
  ...
  switch (rrc->ra_trigger) {
    ...
    case RRC_CONNECTION_REESTABLISHMENT:
      ...
      nr_pdcp_config_set_security(rrc->ue_id, srb_id, 0, NULL, NULL, NULL,false);
      // resume SRB1
                                                  ...
                                                  const nr_pdcp_integrity_data_t *msg_integrity)
{
  ...
  if (!integrity_pass) {
    ...
    for (int i = 1; i < NR_NUM_SRB; i++) {
      if (ue_rrc->Srb[i] == RB_ESTABLISHED)
        nr_pdcp_config_set_security(ue_rrc->ue_id, i, security_mode, NULL, NULL, NULL, false);
    }
                                                 ...
                                                 const NR_RRCReestablishment_t *rrcReestablishment)
{
  ...
  nr_pdcp_config_set_security(rrc->ue_id, srb_id, security_mode, kRRCenc, kRRCint, kUPenc,false);
