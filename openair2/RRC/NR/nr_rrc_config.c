...
static int get_nb_pucch2_per_slot(const NR_ServingCellConfigCommon_t *scc, int bwp_size)
{
  ...
  int NB_UE_per_gNB = fc_nb_ue_per_bwp(bwp_size);
  int max_csi_reports = NB_UE_per_gNB << 1; // 2 reports per UE (RSRP and RI-PMI-CQI)
  ...
  // and NB_UE_per_gNB prbs for PUCCH1
  AssertFatal((nb_pucch2 * PUCCH2_SIZE) + NB_UE_per_gNB <= bwp_size,
              "Cannot allocate all required PUCCH resources for max number of %d UEs in BWP with %d PRBs\n",
              NB_UE_per_gNB, bwp_size);
  return nb_pucch2;
}
      ...
      if (scs == dl_fs->supportedSubcarrierSpacingDL
        ...
        if (maxMIMO_layers == -1)
          *pdsch_servingcellconfig->ext1->maxMIMO_Layers =
              NR_MAX_SUPPORTED_DL_LAYERS < ue_supported_layers ? NR_MAX_SUPPORTED_DL_LAYERS : ue_supported_layers;
        else
          *pdsch_servingcellconfig->ext1->maxMIMO_Layers =
              maxMIMO_layers < ue_supported_layers ? maxMIMO_layers : ue_supported_layers;
        return;
      }
    }
  }
  ...
  *aset->alpha = NR_Alpha_alpha1;
  ...
  if (use_deltaMCS) {
    ...
    *pusch_Config->pusch_PowerControl->deltaMCS = NR_PUSCH_PowerControl__deltaMCS_enabled;
    ...
    free(pusch_Config->pusch_PowerControl->deltaMCS);
                             ...
                             const NR_UE_NR_Capability_t *uecap)
{
  ...
  ubwp->bwp_Dedicated->pusch_Config =
      config_pusch(pusch_Config, configuration->use_deltaMCS, scc, configuration->force_UL256qam_off ? NULL : uecap);
  ...
  ubwp->bwp_Dedicated->srs_Config =
      get_config_srs(scc, NULL, curr_bwp, uid, bwp_loop + 1, maxMIMO_Layers, configuration->minRXTXTIME, configuration->do_SRS);
...
void configure_dedicated_dl_bwp_ho(NR_ServingCellConfig_t *scd,
                                   const NR_ServingCellConfigCommon_t *scc,
                                   const nr_mac_config_t *configuration)
{
...
void configure_dedicated_ul_bwp_ho(int uid, NR_ServingCellConfig_t *scd, const NR_ServingCellConfigCommon_t *scc,  const nr_mac_config_t *configuration) {
                            ...
                            const NR_ServingCellConfigCommon_t *scc)
{
  ...
  if (configuration->do_SRS && uplinkConfig && uplinkConfig->initialUplinkBWP) {
    ...
    uplinkConfig->initialUplinkBWP->srs_Config =
        get_config_srs(scc, uecap, curr_bwp, uid, 0, maxMIMO_Layers, configuration->minRXTXTIME, configuration->do_SRS);
  }
  ...
  if (UL_BWP_list) {
    for (int i = 0; i < UL_BWP_list->list.count; i++) {
      ...
      ul_bwp->bwp_Dedicated->srs_Config =
          get_config_srs(scc, uecap, bwp_size, uid, i + 1, maxMIMO_Layers, configuration->minRXTXTIME, configuration->do_SRS);
    }
  }
...
int encode_cellGroupConfig(NR_CellGroupConfig_t *cellGroupConfig, uint8_t *buffer, int max_buffer_size)
{
  ...
  if(!(enc_rval.encoded > 0 && (enc_rval.encoded <= max_buffer_size * 8)))
  {
    LOG_E(NR_RRC, "Assert Fatal ASN1 message CellGroupConfig encoding failed (%s, %lu)!\n",enc_rval.failed_type->name,enc_rval.encoded);
    return 0;
  }
  return (enc_rval.encoded + 7) / 8;
}
