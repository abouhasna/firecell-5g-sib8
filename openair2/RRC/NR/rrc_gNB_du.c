...
extern void set_cu_collector_cell_state_inactive(uint64_t nr_cell_id);
...
/* Removed STATIC keyword for Cu Collector Task*/
...
void rrc_gNB_process_f1_setup_req(f1ap_setup_req_t *req, sctp_assoc_t assoc_id)
{
  ...
  *du->setup_req = *req;
  ...
  du->sib1 = sib1;
  du->mtc = mtc;
  du->cell_state = CELL_STATE_ACTIVE;
  ...
  rrc->max_num_UEs = fc_nb_ue_per_bwp(bwp_size);
  LOG_I(NR_RRC, "DU %ld (%s) added to RRC, RRC MAX_UES %ld for BWP %d \n", req->gNB_DU_id, req->gNB_DU_name, rrc->max_num_UEs,bwp_size);
...
void rrc_CU_process_f1_lost_connection(gNB_RRC_INST *rrc, f1ap_lost_connection_t *lc, sctp_assoc_t assoc_id)
{
  ...
  uint64_t nr_cell_id = du->setup_req->cell[0].info.nr_cellid;
  set_cu_collector_cell_state_inactive(nr_cell_id);
  ASN_STRUCT_FREE(asn_DEF_NR_MIB, du->mib);
  ASN_STRUCT_FREE(asn_DEF_NR_SIB1, du->sib1);
  ASN_STRUCT_FREE(asn_DEF_NR_MeasurementTimingConfiguration, du->mtc);
