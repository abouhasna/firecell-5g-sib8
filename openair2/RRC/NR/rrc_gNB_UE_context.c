...
rrc_gNB_ue_context_t *rrc_gNB_get_ue_context_by_rnti(gNB_RRC_INST *rrc_instance_pP, sctp_assoc_t assoc_id, rnti_t rntiP)
{
  ...
  RB_FOREACH(ue_context_p, rrc_nr_ue_tree_s, &(rrc_instance_pP->rrc_ue_head)) {
    // TODO: firecell - validate fixes
    // if (ue_context_p->ue_context.is_nsa)
    //   return NULL;
...
//------------------------------------------------------------------------------
{
  ...
  rrc_instance_pP->num_active_UEs --;
  if(rrc_instance_pP->num_active_UEs < 0)
    rrc_instance_pP->num_active_UEs = 0;
  LOG_I(NR_RRC, "Removed UE context. remaining connected UEs: %ld\n", rrc_instance_pP->num_active_UEs);
}
...
//-----------------------------------------------------------------------------
{
  ...
  rrc_instance_pP->num_active_UEs++;
  LOG_I(NR_RRC,
        "Created new UE context: CU UE ID %u DU UE ID %u (rnti: %04x, random ue id %lx) no_of_connected_ues: %ld\n",
        ue->rrc_ue_id,
        du_ue_id,
        ue->rnti,
