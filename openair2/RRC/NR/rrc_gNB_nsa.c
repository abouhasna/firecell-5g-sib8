...
/** ToDo: Below API is need for NSA. Currently spcell dedicated config are used from MAC context.
This may lead to some expected crash case as part UE removal. To avoid this we need to use
dedicated SPCell config and this info should be used to as part add_nsa_user. Facing some issue in clone.
As of now commenting this API. need to work on this in future.
*/
/** NSA_DEBUG **
static void clone_spCellConfigDedicated (const NR_ServingCellConfig_t *orig, rrc_gNB_ue_context_t *ue_context_pP)
{
  uint64_t size;
  size = 1 + sizeof(NR_ServingCellConfig_t);
  uint8_t buf[size];
      LOG_E(RRC, "size of ServCellcfg struct is %d\n", size);
  xer_fprint(stdout, &asn_DEF_NR_ServingCellConfig, orig);
  asn_enc_rval_t enc_rval = uper_encode_to_buffer(&asn_DEF_NR_ServingCellConfig, NULL, orig, buf, sizeof(buf));
  AssertFatal(enc_rval.encoded > 0, "could not clone Spcell dedicated cfg: problem while encoding\n");
  asn_dec_rval_t dec_rval = uper_decode(NULL, &asn_DEF_NR_ServingCellConfig, (void
**)&ue_context_pP->ue_context.spCellConfig->spCellConfigDedicated, buf, enc_rval.encoded, 0, 0); AssertFatal(dec_rval.code == RC_OK
&& dec_rval.consumed == enc_rval.encoded, "could not clone Spcell dedicated cfg: problem while decodung\n");
}
*/
...
void rrc_parse_ue_capabilities(gNB_RRC_INST *rrc, NR_UE_CapabilityRAT_ContainerList_t *UE_CapabilityRAT_ContainerList, x2ap_ENDC_sgnb_addition_req_t *m, NR_CG_ConfigInfo_IEs_t *cg_config_info)
{
  ...
  //TODO: firecell - validate changes
  // UE->spCellConfig = (NR_SpCellConfig_t *)calloc(1, sizeof(struct NR_SpCellConfig));
  // UE->spCellConfig->spCellConfigDedicated = RC.nrmac[0]->common_channels[0].pre_ServingCellConfig;
  // ToDo: Refer function definition for further details
  //  ue_context_p->ue_context.spCellConfig->spCellConfigDedicated = NULL;
  //  clone_spCellConfigDedicated(RC.nrmac[0]->common_channels[0].pre_ServingCellConfig, ue_context_p);
...
#ifdef NOT_USED
static void clone_serv_cell_cfg_cmn(const NR_ServingCellConfigCommon_t *orig, rrc_gNB_ue_context_t *ue_context_pP)
{
  uint8_t buf[20000];
  asn_enc_rval_t enc_rval = uper_encode_to_buffer(&asn_DEF_NR_ServingCellConfigCommon, NULL, orig, buf, sizeof(buf));
  AssertFatal(enc_rval.encoded > 0, "could not clone Serving cell common cfg: problem while encoding\n");
  asn_dec_rval_t dec_rval = uper_decode(NULL,
                                        &asn_DEF_NR_ServingCellConfigCommon,
                                        (void **)&ue_context_pP->ue_context.servcellcmnconfig,
                                        buf,
                                        enc_rval.encoded,
                                        0,
                                        0);
  AssertFatal(dec_rval.code == RC_OK && dec_rval.consumed == enc_rval.encoded,
              "could not clone  NR_ServingCellConfigCommon: problem while decoding\n");
}
#endif
void rrc_add_nsa_user(gNB_RRC_INST *rrc, rrc_gNB_ue_context_t *ue_context_p, x2ap_ENDC_sgnb_addition_req_t *m)
{
  ...
  // if (sccc != NULL) {
  //   clone_serv_cell_cfg_cmn(sccc, ue_context_p);
  // }
  // // ToDo: Refer function definition for further details
  // /*if(scd != NULL)
  // {
  //   clone_spCellConfigDedicated(RC.nrmac[0]->common_channels[0].pre_ServingCellConfig,ue_context_p);
  // } */
  // UE->secondaryCellGroup = get_default_secondaryCellGroup(ue_context_p->ue_context.servcellcmnconfig,
  //                                                         scc,
  //                                                         UE->UE_Capability_nr,
  //                                                         1,
  //                                                         1,
  ...
  // /* Info: No Need to specifically reset the status as part remove UE. entire data will be flushed*/
  // ue_context_p->ue_context.is_nsa = true;
  // static uint32_t rrc_ue_id;
  // rrc_ue_id++;
  ...
  // ue_context_p->ue_context.rrc_ue_id = rrc_ue_id;
  // LOG_I(RRC,
...
void rrc_remove_nsa_user(gNB_RRC_INST *rrc, int rnti) {
  ...
  gtpv1u_delete_all_s1u_tunnel(rrc->module_id, (uint64_t)ue_context->ue_context.rrc_ue_id);

  // ue_context->ue_context.spCellConfig->spCellConfigDedicated = NULL;
  /* remove context */
