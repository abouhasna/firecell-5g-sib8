...
//-----------------------------------------------------------------------------
{
  ...
  *sr_ProhibitTimer_r9 = 0;   // SR tx on PUCCH, Value in number of SR period(s). Value 0 = no timer for SR, Value 2 = 2*SR
  ...
  if (ue_context_pP->ue_context.does_nr) {
    ...
    LOG_I(RRC, "NR meas object band is: %u\n", rrc_inst->nr_band);
  ...
  *rsrp = 20;
  ...
  measGapConfig->present=LTE_MeasGapConfig_PR_setup;
  measGapConfig->choice.setup.gapOffset.present=LTE_MeasGapConfig__setup__gapOffset_PR_gp0;
  quantityConfig = CALLOC(1, sizeof(*quantityConfig));
  memset((void *)quantityConfig, 0, sizeof(*quantityConfig));
  quantityConfig->quantityConfigEUTRA = CALLOC(1, sizeof(struct LTE_QuantityConfigEUTRA));
  memset((void *)quantityConfig->quantityConfigEUTRA, 0, sizeof(*quantityConfig->quantityConfigEUTRA));
  ...
  *quantityConfig->quantityConfigEUTRA->filterCoefficientRSRQ = LTE_FilterCoefficient_fc4;
  ...
  size = do_RRCConnectionReconfiguration(ctxt_pP,
                                         ...
                                         measurements_enabled ? (LTE_MeasGapConfig_t *)measGapConfig : NULL,
                                         (LTE_MobilityControlInfo_t *) NULL,
                                         (LTE_SecurityConfigHO_t *) NULL,
...
//-----------------------------------------------------------------------------
{
  ...
  if(measResults2->measId == 4)
  {
    /* Received MR with A3 Event[i.e. serving become worst and neighbour becomes better]As of now X2 is established only with NSA cell. if we allow this HO based on the X2 availability, 
     *	it may leads to some unexpected issue since there is no X2 inteface to another LTE cell. In future this can be removed if multi LTE cell environment is supported */
    LOG_E(RRC,"Error:UE triggers A3 event we dont support inter LTE HO ignoreing the MR\n");
