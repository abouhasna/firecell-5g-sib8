  ...
  if (dl_info->dci_ind && dl_info->dci_ind->number_of_dcis) {
    ...
    for (int i = 0; i < dl_info->dci_ind->number_of_dcis; i++) {
      ...
      nr_dci_format_t dci_format =
          handle_dci(mac, dl_info->gNB_index, dl_info->frame, dl_info->slot, dl_info->dci_ind->dci_list + i);
...
*/
void sl_nr_process_rx_ind(int ue_id, uint32_t frame, uint32_t slot, sl_nr_ue_mac_params_t *sl_mac, sl_nr_rx_indication_t *rx_ind)
{
