...
static  MessageDef      *ngap_reg_msg_p = NULL;
uint32_t gNB_app_register(uint32_t gnb_id_start, uint32_t gnb_id_end)//, const Enb_properties_array_t *enb_properties)
{
  ...
  for (uint32_t gnb_id = gnb_id_start; (gnb_id < gnb_id_end) ; gnb_id++) {
    {
      ...
      if(get_softmodem_params()->sa){
        ...
        /* Currenlty gNB connects only 1 AMF, store the NGAP_REGISTER request for future retries */
        if (ngap_reg_msg_p == NULL) {
          ngap_reg_msg_p = itti_alloc_new_message(TASK_GNB_APP, 0, NGAP_REGISTER_GNB_REQ);
	  
          RCconfig_NR_NG(msg_p, gnb_id);
    
          memcpy((void *)ngap_reg_msg_p, (void *)msg_p, sizeof(ngap_register_gnb_req_t));
        } else {
          //Re-trying the NG_SETUP use the stored request message
          memcpy((void *)msg_p, (void *)ngap_reg_msg_p, sizeof(ngap_register_gnb_req_t));
        }
...
*/
int rrc_gNB_trigger_release_all() {
  ...
  RB_FOREACH(ue_context_p, rrc_nr_ue_tree_s, &(RC.nrrrc[0]->rrc_ue_head)) {
    ...
    LOG_E(GNB_APP,"RRC Release triggered for UE %u\n", UE->rrc_ue_id);
  }
...
*/
    ...
    case NGAP_DEREGISTERED_GNB_IND:
      LOG_W(GNB_APP, "[gNB %ld] Received %s: associated AMF %d\n", 
            instance, msg_name, NGAP_DEREGISTERED_GNB_IND(msg_p).nb_amf);

      // Check if at least gNB is registered with one AMF
      if (NGAP_DEREGISTERED_GNB_IND(msg_p).nb_amf > 0) {
        LOG_I(GNB_APP, "[gNB %ld] Received %s: associated AMF %d\n",
              instance, msg_name, NGAP_DEREGISTERED_GNB_IND(msg_p).nb_amf);
      } else {
        uint32_t not_associated = 0;

        LOG_W(GNB_APP,
              " %d gNB %s not associated with a AMF, retrying registration in %d seconds ...\n",
              not_associated,
              not_associated > 1 ? "are" : "is",
              GNB_REGISTER_RETRY_DELAY);
        rrc_gNB_trigger_release_all();
        // Restart the gNB registration process in GNB_REGISTER_RETRY_DELAY seconds
        if (timer_setup(GNB_REGISTER_RETRY_DELAY,
                        0,
                        TASK_GNB_APP,
                        INSTANCE_DEFAULT,
                        TIMER_ONE_SHOT,
          ...
          gNB_app_register(gnb_id_start, gnb_id_end); //, gnb_properties_p);
        } else {
          LOG_I(GNB_APP, " Started GNB_REGISTER_RETRY_DELAY: timer_id %ld\n", gnb_register_retry_timer_id);
        }
      }
      break;
    ...
    case TIMER_HAS_EXPIRED:
      ...
      //if (TIMER_HAS_EXPIRED (msg_p).timer_id == gnb_register_retry_timer_id)
       {
         // Restart the registration process
         LOG_I(GNB_APP, " Starting Register NGAP \n");

          uint32_t                        gnb_nb = RC.nb_nr_inst; 
          uint32_t                        gnb_id_start = 0;
          uint32_t                        gnb_id_end = gnb_id_start + gnb_nb;
          gNB_app_register(gnb_id_start, gnb_id_end);
      }
