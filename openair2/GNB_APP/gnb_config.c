...
#endif
#include "openair2/COLLECTOR/cu_collector.h"
...
void RCconfig_nr_macrlc(configmodule_interface_t *cfg)
{
  ...
  config.do_release_on_assert= *GNBParamList.paramarray[0][GNB_CONFIG_UE_RELEASE_IDX].iptr;
  LOG_I(GNB_APP, "UE release flag: %d\n", config.do_release_on_assert);
  ...
  if (MacRLC_ParamList.numelt > 0) {
    ...
    for (j = 0; j < RC.nb_nr_macrlc_inst; j++) {
      ...
      if (strcmp(*(MacRLC_ParamList.paramarray[j][MACRLC_NR_SCHED_MODE_IDX].strptr), "fairRR") == 0) {
        RC.nrmac[j]->scheduler_mode = NR_SCHED_MODE_FAIR_RR;
        RC.nrmac[j]->nr_fr1_dlsch_algo = nr_fr1_dlsch_pf;
        RC.nrmac[j]->nr_fr1_ulsch_algo = nr_fr1_ulsch_pf;
        LOG_I(NR_MAC, "Propotional Fair NR Scheduler is ACTIVE\n");
      } else if ((strcmp(*(MacRLC_ParamList.paramarray[j][MACRLC_NR_SCHED_MODE_IDX].strptr), "fcPriorityBased") == 0) ||
                 (get_softmodem_params()->qos_sched)) {
        RC.nrmac[j]->scheduler_mode = NR_SCHED_MODE_QOS;
        /* Set the scheduling algorithm for DL */
        RC.nrmac[j]->nr_fr1_dlsch_algo = nr_fr1_dlsch_qos;
        RC.nrmac[j]->nr_fr1_ulsch_algo = nr_fr1_ulsch_qos;
        LOG_I(NR_MAC, "QoS based NR Scheduler is ACTIVE\n");
      } else {
        RC.nrmac[j]->scheduler_mode = NR_SCHED_MODE_FAIR_RR;
        RC.nrmac[j]->nr_fr1_dlsch_algo = nr_fr1_dlsch_pf;
        RC.nrmac[j]->nr_fr1_ulsch_algo = nr_fr1_ulsch_pf;
    ...
    int bwp_size = (req->cell[0].info.mode == F1AP_MODE_TDD ? req->cell[0].info.tdd.tbw.nrb : req->cell[0].info.fdd.dl_tbw.nrb);
    RC.nrmac[0]->max_num_UEs = fc_nb_ue_per_bwp(bwp_size);
    LOG_I(NR_MAC, "Max number of UEs for BWP size %d: %d\n", bwp_size, RC.nrmac[0]->max_num_UEs);
...
void RCconfig_NRRRC(gNB_RRC_INST *rrc)
{
  ...
  if (num_gnbs > 0) {
    ...
    rrc->node_id        = gnb_id;
    rrc->do_release_on_assert = *(GNBParamList.paramarray[0][GNB_CONFIG_UE_RELEASE_IDX].iptr);
...
void get_du_collector_cfg(struct du_collector_config *cfg)
{
  if (!cfg) {
    LOG_E(FC_COLLECTOR, "du config is null\n");
    assert(0);
  }
  paramdef_t DU_Collector_cfg[] = COLLECTOR_GLOBAL_PARAMS_DU_DESC;

  if (config_get(config_get_if(), DU_Collector_cfg, sizeofArray(DU_Collector_cfg), CONFIG_STRING_COLLECTOR_DU) < 0) {
    LOG_E(FC_COLLECTOR, "can not read DU config\n");
    assert(0);
  }
  if (!DU_Collector_cfg[DU_COLLECTOR_HTTP_PORT_IDX].uptr) {
    LOG_E(FC_COLLECTOR, "please set DU http port\n");
    assert(0);
  }
  if (!DU_Collector_cfg[DU_COLLECTOR_HTTP_HOST_IDX].strptr) {
    LOG_E(FC_COLLECTOR, "please set DU http host\n");
    assert(0);
  }
  if (!DU_Collector_cfg[DU_COLLECTOR_REDIS_PORT_IDX].uptr) {
    LOG_E(FC_COLLECTOR, "please set DU telnet port\n");
    assert(0);
  }
  if (!DU_Collector_cfg[DU_COLLECTOR_REDIS_HOST_IDX].strptr) {
    LOG_E(FC_COLLECTOR, "please set DU telnet host\n");
    assert(0);
  }

  cfg->http_port = (uint16_t) * (DU_Collector_cfg[DU_COLLECTOR_HTTP_PORT_IDX].uptr);
  cfg->redis_port = (uint16_t) * (DU_Collector_cfg[DU_COLLECTOR_REDIS_PORT_IDX].uptr);

  if (strlen(*(DU_Collector_cfg[DU_COLLECTOR_HTTP_HOST_IDX].strptr)) < (sizeof(cfg->http_host) - 1)) {
    strncpy(cfg->http_host, *(DU_Collector_cfg[DU_COLLECTOR_HTTP_HOST_IDX].strptr), sizeof(cfg->http_host) - 1);
    cfg->http_host[sizeof(cfg->http_host) - 1] = '\0';
  } else {
    LOG_E(FC_COLLECTOR, "please check the http host address \n");
    assert(0);
  }
  if (strlen(*(DU_Collector_cfg[DU_COLLECTOR_REDIS_HOST_IDX].strptr)) < (sizeof(cfg->redis_host) - 1)) {
    strncpy(cfg->redis_host, *(DU_Collector_cfg[DU_COLLECTOR_REDIS_HOST_IDX].strptr), sizeof(cfg->redis_host) - 1);
    cfg->redis_host[sizeof(cfg->redis_host) - 1] = '\0';
  } else {
    LOG_E(FC_COLLECTOR, "please check the redis host address \n");
    assert(0);
  }
}

void get_cu_collector_cfg(struct cu_collector_config *cfg)
{
  if (!cfg) {
    LOG_E(FC_COLLECTOR, "cu config is null\n");
    assert(0);
  }
  paramdef_t CU_Collector_cfg[] = COLLECTOR_GLOBAL_PARAMS_CU_DESC;

  if (config_get(config_get_if(), CU_Collector_cfg, sizeofArray(CU_Collector_cfg), CONFIG_STRING_COLLECTOR_CU) < 0) {
    LOG_E(FC_COLLECTOR, "can not read CU config\n");
    assert(0);
  }
  if (!CU_Collector_cfg[CU_COLLECTOR_HTTP_PORT_IDX].uptr) {
    LOG_E(FC_COLLECTOR, "please set http port\n");
    assert(0);
  }
  if (!CU_Collector_cfg[CU_COLLECTOR_HTTP_HOST_IDX].strptr) {
    LOG_E(FC_COLLECTOR, "please set http host\n");
    assert(0);
  }
  if (!CU_Collector_cfg[CU_COLLECTOR_TELNET_PORT_IDX].uptr) {
    LOG_E(FC_COLLECTOR, "please set telnet port\n");
    assert(0);
  }
  if (!CU_Collector_cfg[CU_COLLECTOR_TELNET_HOST_IDX].strptr) {
    LOG_E(FC_COLLECTOR, "please set telnet host\n");
    assert(0);
  }

  cfg->http_port = (uint64_t) * (CU_Collector_cfg[CU_COLLECTOR_HTTP_PORT_IDX].uptr);
  cfg->telnet_port = (uint64_t) * (CU_Collector_cfg[CU_COLLECTOR_TELNET_PORT_IDX].uptr);

  if (strlen(*(CU_Collector_cfg[CU_COLLECTOR_HTTP_HOST_IDX].strptr)) < (sizeof(cfg->http_host) - 1)) {
    strncpy(cfg->http_host, *(CU_Collector_cfg[CU_COLLECTOR_HTTP_HOST_IDX].strptr), sizeof(cfg->http_host) - 1);
    cfg->http_host[sizeof(cfg->http_host) - 1] = '\0';
  } else {
    LOG_E(FC_COLLECTOR, "please check the http host address \n");
    assert(0);
  }
  if (strlen(*(CU_Collector_cfg[CU_COLLECTOR_TELNET_HOST_IDX].strptr)) < (sizeof(cfg->telnet_host) - 1)) {
    strncpy(cfg->telnet_host, *(CU_Collector_cfg[CU_COLLECTOR_TELNET_HOST_IDX].strptr), sizeof(cfg->telnet_host) - 1);
    cfg->telnet_host[sizeof(cfg->telnet_host) - 1] = '\0';
  } else {
    LOG_E(FC_COLLECTOR, "please check the telnet host address \n");
    assert(0);
  }
}
