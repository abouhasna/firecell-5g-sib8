...
#include "LAYER2/NR_MAC_gNB/gNB_scheduler_dlsch.h"
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  while (remainUEs> 0 && n_rb_sched >= min_rbSize && iterator->UE != NULL) {
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    //ensure that the minimum RB size is less than the maximum RB size
    min_rbSize= (min(max_rbSize, min_rbSize));
...
*/
void nr_fr1_dlsch_pf(NR_Scheduling_context_t *sched_context) {
  /* Retrieve amount of data to sendd for this UE */
  nr_store_dlsch_buffer(sched_context->module_id, sched_context->frame, sched_context->slot);
  /* proportional fair scheduling algorithm */
  pf_dl(sched_context->module_id,
        sched_context->frame,
        sched_context->slot,
...
static int check_ue_sched_valid_dl(NR_UE_info_t *UE,gNB_MAC_INST *nrmac,NR_sched_pdsch_t *sched_pdsch)
{
   int rbStop = 0;
   int rbStart = 0;
   get_start_stop_allocation(nrmac, UE, &rbStart, &rbStop);
   
   int NPRB= sched_pdsch->rbSize;
   int RBstart= sched_pdsch->rbStart - rbStart;
   int BWPsize= rbStop - rbStart + 1;
    
   if (NPRB <= 0 || (NPRB + RBstart > BWPsize))
   {
     LOG_E(NR_MAC, "check_ue_sched_valid_dl Illegal NPRB/RBstart Configuration (%d,%d) for BWPsize %d UE %x \n",
                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  /* Loop through the list of all unsorted UEs */
  UE_iterator(UE_info->list, UE) {
    ...
    if (check_ue_sched_valid_dl(UE,gNB_mac,sched_pdsch) < 0)
      continue;
