...
size_t dump_mac_stats(gNB_MAC_INST *gNB, char *output, size_t strlen, bool reset_rsrp)
{
  ...
  UE_iterator(gNB->UE_info.list, UE) {
    ...
    output += snprintf(output,
                       ...
                       ", dlsch_errors %"PRIu64", pucch0_DTX %d, BLER %.5f MCS (%d) %d NPRB %d\n",
                       stats->dl.errors,
                       stats->pucch0_DTX,
                       sched_ctrl->dl_bler_stats.bler,
                       UE->current_DL_BWP.mcsTableIdx,
                       sched_ctrl->dl_bler_stats.mcs,
                       UE->mac_stats.NPRB_DL);
    if (reset_rsrp) {
    ...
    output += snprintf(output,
                       ...
                       "UE %04x: Invalid_RSRP %d, Normalized_power %d\n", UE->rnti, 
                       UE->mac_stats.invalid_RSRP, UE->mac_stats.normalized_power);
  ...
  if (RC.nb_nr_macrlc_inst > 0) {
    ...
    for (i = 0; i < RC.nb_nr_macrlc_inst; i++) {
      ...
      if (get_softmodem_params()->phy_test) {
        ...
        if (get_softmodem_params()->qos_sched) {
          RC.nrmac[i]->nr_fr1_dlsch_algo = nr_fr1_dlsch_qos;
          RC.nrmac[i]->nr_fr1_ulsch_algo = nr_fr1_ulsch_qos;

        } else {
          RC.nrmac[i]->nr_fr1_dlsch_algo = nr_fr1_dlsch_pf;
          RC.nrmac[i]->nr_fr1_ulsch_algo = nr_fr1_ulsch_pf;
        }
      }
