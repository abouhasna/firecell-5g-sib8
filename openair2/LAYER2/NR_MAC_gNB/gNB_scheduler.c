...
/** the Current MAC stats collection, it requires Max of 3000 bytes to dump the info 
 * for an one UE. based on no of active UEs at MAC it will be allocated. */ 
#define MAC_STATS_BUFF ( MAX_MOBILES_PER_GNB * 3000)
...
void gNB_dlsch_ulsch_scheduler(module_id_t module_idP, frame_t frame, sub_frame_t slot, NR_Sched_Rsp_t *sched_info)
{
  ...
  if ((slot == 0) && (frame & 127) == 0) {
    char stats_output[MAC_STATS_BUFF] = {0};
    dump_mac_stats(gNB, stats_output, sizeof(stats_output), true);
    LOG_I(NR_MAC, "Frame.Slot %d.%d\n%s\n", frame, slot, stats_output);
#ifdef UE_ALLOC
    LOG_I(NR_MAC, "UE allocation bitmap [%d.%d]: ", frame, slot);
    for (long i = 0; i < MAX_MOBILES_PER_GNB; i++) {
      printf("%ld ", (gNB->slot_alloc_bitmap >> i) & 1);
    }
    fflush(stdout);
    LOG_I(NR_MAC, "\n");
#endif
