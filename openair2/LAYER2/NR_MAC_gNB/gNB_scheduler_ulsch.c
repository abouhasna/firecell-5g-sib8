...
#include "LAYER2/NR_MAC_gNB/gNB_qos_scheduler_ulsch.h"
...
static int estimate_ul_buffer_long_bsr(const NR_BSR_LONG *bsr)
{
  LOG_D(NR_MAC,
        ...
        bsr->LcgID1,
        bsr->LcgID0);
  bool bsr_active[8] = {bsr->LcgID0 != 0,
                        bsr->LcgID1 != 0,
                        bsr->LcgID2 != 0,
                        bsr->LcgID3 != 0,
                        bsr->LcgID4 != 0,
  ...
  while (!done && pdu_len > 0){
    ...
    switch(rx_lcid){
      ...
      case UL_SCH_LCID_CCCH1:
        ...
        /** Note: Here, the gNB is receiving the CCCH message for active UE RNTIs.
         -  Forwarding the same to CU/RRC Since there is a CCCH messsage called RRCSystemInfoRequest 
	 -  which can be sent by the UE.
         */
        if(UE->CellGroup!=NULL){
          LOG_I(NR_MAC, "Received the CCCH message from RNTI %04x Forward it to RRC \n", UE->rnti);
        }
        
        if (prepare_initial_ul_rrc_message(RC.nrmac[module_idP], UE) ||(UE->CellGroup!=NULL)) {
          mac_rlc_data_ind(module_idP,
                          UE->rnti,
                          module_idP,
                          frameP,
      ...
      case UL_SCH_LCID_DTCH ... (UL_SCH_LCID_DTCH + 28):
        ...
        sdus += 1;
        // Update UL BWF
        nr_update_uplink_bwf(rx_lcid, sched_ctrl);
...
static bool get_UE_waiting_CFRA_msg3(const NR_UE_info_t *UE, const gNB_MAC_INST *gNB_mac, const int CC_id, const frame_t frame, const sub_frame_t slot)
{
  ...
  for (int i = 0; i < NR_NB_RA_PROC_MAX; i++) {
    ...
    if (ra->cfra == true && ra->ra_state == nrRA_WAIT_Msg3 && frame == ra->Msg3_frame && slot == ra->Msg3_slot && UE->rnti == ra->rnti) {
    ...
    for (int i = 0; i < NR_NB_RA_PROC_MAX; ++i) {
      ...
      if (ra->cfra) {
        ...
        if (crnti != 0) { // 3GPP TS 38.321 Table 7.1-1: RNTI values, RNTI 0x0000: N/A
          ...
          int prev_msg4acked = UE->Msg4_ACKed;
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (nrmac->tdd_config.is_tdd) { // Force the default transmission in a full slot as early as possible in the UL portion of TDD
      ...
      last_ul_slot = 1 + scc->tdd_UL_DL_ConfigurationCommon->pattern1.nrofDownlinkSlots;
    } else {
  ...
  while (remainUEs> 0 && n_rb_sched >= min_rb && iterator->UE != NULL) {
    ...
    {
      int tx_power = nr_ue_max_mcs_min_rb(current_BWP->scs,
                                          sched_ctrl->ph,
                                          sched_pusch,
                                          current_BWP,
                                          min_rb,
                                          ...
                                          &max_rbSize,
                                          &sched_pusch->mcs);
      if (sched_ctrl->ph < tx_power) {
        LOG_D(NR_MAC,
              "UE %x Normalized power %d based on current resources (RBs %d, MCS %d) exceed reported PHR %d (normalized value)\n",
              iterator->UE->rnti,
              tx_power,
              max_rbSize,
              sched_pusch->mcs,
              sched_ctrl->ph);
        iterator->UE->mac_stats.normalized_power++;
      }
    }
    ...
    if (sched_pusch->mcs == 0)
    {
      /* FC-Fix: The nr_ue_max_mcs_min_rb adjusted the MCS = 0 due to PH limitation, 
       * temprory fix to check with MCS=6 and minRB scheduled if UE can still communicate
       */
      sched_ctrl->ul_bler_stats.mcs = 6;
      sched_pusch->mcs = 6;
      LOG_D(NR_MAC,
              "UE %x Setting MCS to 6 current resources (RBs %d, MCS %d) exceed reported PHR %d (normalized value)\n",
              iterator->UE->rnti,
              max_rbSize,
              sched_pusch->mcs,
              sched_ctrl->ph);
...
/**
 * @brief Proportional-fair scheduling algorithms
*/
void nr_fr1_ulsch_pf(NR_Scheduling_context_t *sched_context) {
  /* proportional fair scheduling algorithm */
  pf_ul(sched_context->module_id,
        sched_context->frame,
        sched_context->slot,
        sched_context->UE_list,
        sched_context->max_sched_ues,
        sched_context->n_rb_sched,
        sched_context->rballoc_mask);
}
...
static bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  max_sched_ues = min(max_sched_ues, MAX_DCI_CORESET);
   ...
   /* Call the dlsch scheduling algorithm */
  NR_Scheduling_context_t sched_context = {
    .frame = frame,
    .module_id = module_id,
    .slot = slot,
    .UE_list = nr_mac->UE_info.list,
    .max_sched_ues = max_sched_ues,
    .n_rb_sched = len,
    .rballoc_mask = rballoc_mask
  };
  RC.nrmac[module_id]->nr_fr1_ulsch_algo(&sched_context);
  return true;
}
...
static int check_ue_sched_valid_ul(NR_UE_info_t *UE,gNB_MAC_INST *nrmac, NR_sched_pusch_t *sched_pusch)
{
   int NPRB= sched_pusch->rbSize;
   int RBstart= sched_pusch->rbStart;
   int BWPsize= UE->current_UL_BWP.BWPSize;
    
   if (NPRB <= 0 || (NPRB + RBstart > BWPsize))
   {
     LOG_E(NR_MAC, "check_ue_sched_valid_ul Illegal NPRB/RBstart Configuration (%d,%d) for BWPsize %d UE %x \n",
              NPRB, RBstart, BWPsize, UE->rnti);
     return (-1);
   }
   return 0;
}
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  UE_iterator( UE_info->list, UE) {
    ...
    if (check_ue_sched_valid_ul(UE,nr_mac,sched_pusch) < 0)
      continue;
