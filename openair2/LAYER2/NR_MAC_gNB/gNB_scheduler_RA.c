      ...
      for (int fdm_index=0; fdm_index < fdm; fdm_index++) { // one structure per frequency domain occasion
        for (int td_index=0; td_index<N_t_slot; td_index++) {
          ...
          if((prach_occasion_id < cc->total_prach_occasions) && (td_index == 0)){
            ...
            prach_pdu->num_cs =
                get_NCS(rach_ConfigGeneric->zeroCorrelationZoneConfig, format0, rach_ConfigCommon->restrictedSetConfig);
                         ...
                         int16_t timing_offset)
{
  ...
  // check if gNB has max UE connected Igore RA procedure
  if(nr_mac->num_active_UEs >= nr_mac->max_num_UEs) {
    LOG_E(NR_MAC, "Ignoring: %4d.%2d RA procedure for preamble index %d: gNB has max UE connected\n",
                                 ...
                                 tdd_config_t *tdd_config)
{
  ...
  // TDD
  DevAssert(tdd_config->is_tdd != false);
                             ...
                             nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  LOG_A(NR_MAC, "Msg2 startSymbolIndex.nrOfSymbols %d.%d\n", tda_info.startSymbolIndex, tda_info.nrOfSymbols);
...
void nr_clear_ra_proc(NR_RA_t *ra)
{
  ...
  ra->timing_offset = 0;
  ra->msg3_round = 0;
  if (ra->CellGroup != NULL && ra->CellGroup->spCellConfig != NULL && ra->CellGroup->spCellConfig->spCellConfigDedicated != NULL) {
    struct NR_ServingCellConfig *spcellConfigDedicated = ra->CellGroup->spCellConfig->spCellConfigDedicated;
    if (spcellConfigDedicated->firstActiveDownlinkBWP_Id != NULL) {
      free(spcellConfigDedicated->firstActiveDownlinkBWP_Id);
      spcellConfigDedicated->firstActiveDownlinkBWP_Id = NULL;
    }

    if (spcellConfigDedicated->defaultDownlinkBWP_Id != NULL) {
      free(spcellConfigDedicated->defaultDownlinkBWP_Id);
      spcellConfigDedicated->defaultDownlinkBWP_Id = NULL;
    }

    if (spcellConfigDedicated->uplinkConfig != NULL && spcellConfigDedicated->uplinkConfig->firstActiveUplinkBWP_Id != NULL) {
      free(spcellConfigDedicated->uplinkConfig->firstActiveUplinkBWP_Id);
      spcellConfigDedicated->uplinkConfig->firstActiveUplinkBWP_Id = NULL;
    }
  }
  if(ra->cfra == false) {
    ra->rnti = 0;
