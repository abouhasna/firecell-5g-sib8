...
static NR_UE_harq_t *find_harq(frame_t frame, sub_frame_t slot, NR_UE_info_t * UE, int harq_round_max)
{
  ...
  while ((harq->feedback_frame - frame + 1024) % 1024
             > 512 // harq->feedback_frame < frame, distance of 512 is boundary to decide if feedback_frame is in the past or future
  ...
  if ((frame - harq->feedback_frame + 1024) % 1024
          > 512 // harq->feedback_frame > frame, distance of 512 is boundary to decide if feedback_frame is in the past or future
      || (harq->feedback_frame == frame && harq->feedback_slot > slot)) {
    LOG_W(NR_MAC,
