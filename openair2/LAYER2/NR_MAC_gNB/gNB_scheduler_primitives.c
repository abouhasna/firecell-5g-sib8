...
void mac_remove_nr_ue(gNB_MAC_INST *nr_mac, rnti_t rnti)
{
  ...
  NR_RA_t *ra = &nr_mac->common_channels[0].ra[0];
  NR_UEs_t *UE_info = &nr_mac->UE_info;
  NR_SCHED_LOCK(&UE_info->mutex);
  ...
  if (!UE) {
    ...
    for (int i = 0; i < NR_NB_RA_PROC_MAX; i++, ra++) {
      if (ra->rnti == rnti) {
        LOG_W(NR_MAC, "Still rnti is active at MAC RA buffer: removing the RNTI from RA Buffer %04x \n", rnti);
        nr_clear_ra_proc(ra);
      }
    }
    NR_SCHED_UNLOCK(&UE_info->mutex);
...
void send_initial_ul_rrc_message(int rnti, const uint8_t *sdu, sdu_size_t sdu_len, void *data)
{
  ...
  if (!encoded)
  {
    /* The assert is removed from the encode_cellgroupConfig function.
     * encode cell groups shall not fail, something is really wrong, mark the UE as failed
     * setting a value to ul_failure_timer= 2 ensures UE to be released in the next slot
     * the CU ignores F1_UE_REL_REQ if the UE context is not avaialble and the DU MAC
     * clears its context
     */
    LOG_E(NR_MAC, "send_initial_ul_rrc_message: encode_cellGroupConfig FAILED return for UE RNTI %d\n", UE->rnti);
...
void fc_nr_mac_trigger_ul_failure(NR_UE_sched_ctrl_t *sched_ctrl, NR_SubcarrierSpacing_t subcarrier_spacing, int rnti, int timer_ms)
{
  if (sched_ctrl->ul_failure) {
    /* already running */
    return;
  }
  LOG_I(MAC, "fc_log:Triggering UL failure timer for UE %04x: Timer set for %d ms\n", rnti, timer_ms);
  // Activate the UL failure flag
  sched_ctrl->ul_failure = true;
  // Calculate and set the UL failure timer
  sched_ctrl->ul_failure_timer = timer_ms << subcarrier_spacing;
}
...
void nr_mac_check_ul_failure(const gNB_MAC_INST *nrmac, int rnti, NR_UE_sched_ctrl_t *sched_ctrl)
{
  ...
  if (sched_ctrl->ul_failure_timer == 1) {
    // If we do not have CU UE context, release from DU itself.

    if (!du_exists_f1_ue_data(rnti)) {
      LOG_W(MAC, "UE %04x: request release after UL failure timer expiry but CU context not found\n", rnti);
      nr_mac_trigger_release_timer(sched_ctrl, 1);
      return;
    }
