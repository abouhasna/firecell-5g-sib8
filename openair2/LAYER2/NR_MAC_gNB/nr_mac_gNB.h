...
typedef struct NR_QoS_map_s {
  int fiveQI;
  int priority;
} NR_QoS_map_t;

typedef struct NR_QoS_config_s {
  /// QoS Flow map for current LC
  NR_QoS_map_t qosMap[NR_MAX_NUM_QFI];
  /// Base weight factor for current LC
  uint8_t baseWeightFactor;
  /// Slope factor for current LC
  float  slopeFactor;
} NR_QoS_config_t;

typedef struct NR_DL_LC_s {
  /// LCID
  uint8_t lcid;
   ...
   *  (0 - CCCH and 1 - 3 SRBs, 4 - 32 DTCH (DRBs 1 - 29))
   *  DRB Identity can be any value [1-29]
   */
  /// QoS config for LC
  NR_QoS_config_t qos_config;
} nr_lc_config_t;
...
typedef struct {
  ...
  // Information about the QoS configuration for each LCID/DRB
  // TODO: firecell - remove this use seq_arr NR_QoS_config_t qos_config[NR_MAX_eNUM_LCID - 4][NR_MAX_NUM_QFI]; // 0 -CCCH and 1- 3
  // SRBs(0,1,2)
  bool transmission_stop;

  /// per-LC configuration
  seq_arr_t lc_config;
  /* the current BWF of the UE based on available data and QoS*/
  int bwf_UE;
  /* Uplink BWF for of the UE based on highest
  priority LCID in previous processed SDU */
  int ul_bwf_UE;
} NR_UE_sched_ctrl_t;
...
typedef struct NR_mac_stats {
  ...
  int pusch_snrx10;
  int deltaMCS;
  int NPRB; // UL PRBs allocated for last TTI
  int NPRB_DL; // DL PRBs allocated for last TTI
  int invalid_RSRP;
...
typedef struct {
  ...
  // Current scheduling frame
  frame_t frame;
  // Current scheduling slot
  sub_frame_t slot;
  // List of UE
  NR_UE_info_t **UE_list;
  // Maximum number of UEs that can be scheduled in the slot
  int max_sched_ues;
  // Number of RBs available for allocation
  int n_rb_sched;
  // Pointer to the RB allocation mask
  uint16_t *rballoc_mask;
} NR_Scheduling_context_t;
...
typedef bool (*nr_pp_impl_ul)(module_id_t mod_id,
                              frame_t frame,
                              sub_frame_t slot);
typedef void (*nr_dlsch_algo_dl)(NR_Scheduling_context_t *sched_context);
typedef void (*nr_ulsch_algo_ul)(NR_Scheduling_context_t *sched_context);
...
typedef struct gNB_MAC_INST_s {
  ...
  nr_dlsch_algo_dl nr_fr1_dlsch_algo;
  /// UL scheduling algorithm
  nr_ulsch_algo_ul nr_fr1_ulsch_algo;
  ...
  /// Stats: Bitmap of allocated UEs in current slot
  unsigned long long slot_alloc_bitmap;
  int16_t scheduler_mode;
  pthread_mutex_t sched_lock;
  uint8_t num_active_UEs;
  uint8_t max_num_UEs;
