...
typedef enum {
  NR_PDCP_INT_CHECK_FAIL,
  NR_PDCP_APPLY_INT_FAIL,
  NR_PDCP_COUNT_MISMATCH,
  NR_PDCP_RX_DELIV_COUNT_BIGGER_THAN_RCVD_COUNT,
  INVALID_CAUSE
} nr_pdcp_failure_cause_t;
...
typedef struct nr_pdcp_entity_t {
  ...
  /* test variable to trigger integrity failure */
  bool fc_trigger_integrity_failure;
  bool fc_pdcp_failure;
  void *fc_pdcp_failure_data;
  void (*fc_inform_upper_layer_for_pdcp_failure)(struct nr_pdcp_entity_t *pdcp_entity, nr_pdcp_failure_cause_t cause);
  /* security */
  int has_ciphering;
  int has_integrity;
  int ciphering_algorithm;
