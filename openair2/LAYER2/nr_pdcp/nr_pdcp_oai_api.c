...
void fc_inform_rrc_for_pdcp_failure(nr_pdcp_entity_t *entity, nr_pdcp_failure_cause_t cause)
{
  nr_pdcp_ue_t *ue = (nr_pdcp_ue_t *)entity->fc_pdcp_failure_data;
  if (ue == NULL) {
    LOG_W(PDCP, "UE not found for pdcp failure\n");
    return;
  }
  uint64_t ue_id = ue->ue_id;

  if (entity->fc_pdcp_failure) {
    LOG_W(PDCP, "PDCP failure already reported for UE %lu\n", ue_id);
    return;
  }
  entity->fc_pdcp_failure = true;
  ngran_node_t node_type = get_node_type();
  if (node_type == ngran_gNB_CU || node_type == ngran_gNB_CUCP || node_type == ngran_gNB) {
    LOG_W(PDCP, "PDCP failure reported for UE %lu\n", ue_id);
    MessageDef *message_p = itti_alloc_new_message(TASK_PDCP_GNB, 0, NRRRC_PDCP_FAILURE);

    message_p->ittiMsg.nr_rrc_pdcp_failure.rrc_ue_id = ue_id;
    message_p->ittiMsg.nr_rrc_pdcp_failure.cause = cause;
    itti_send_msg_to_task(TASK_RRC_GNB, INSTANCE_DEFAULT, message_p);

  } else if (node_type == ngran_gNB_CUUP) {
    LOG_W(PDCP, "CUUP - PDCP failure reported for UE %lu\n", ue_id);
  } else {
    LOG_E(PDCP, "Invalid node type for pdcp %d\n", node_type);
  }
}
             ...
             unsigned char *integrity_key)
{
  ...
  if (nr_pdcp_get_rb(ue, srb_id, true) != NULL) {
    ...
    fc_set_pdcp_failure_callback(pdcp_srb, fc_inform_rrc_for_pdcp_failure, ue);
             ...
             unsigned char *integrity_key)
{
  ...
  if (nr_pdcp_get_rb(ue, drb_id, false) != NULL) {
    ...
    bool qfi_exists = (qfi!=-1);
    pdcp_drb = new_nr_pdcp_entity(NR_PDCP_DRB_AM,
                                  is_gnb,
                                  drb_id,
                                  sdap_entity->pdusession_id,
                                  qfi_exists ? sdap_entity->qfi2drb_table[qfi].has_sdap_rx : false,
                                  qfi_exists ? sdap_entity->qfi2drb_table[qfi].has_sdap_tx : false,
                                  deliver_sdu_drb,
                                  ue,
    ...
    fc_set_pdcp_failure_callback(pdcp_drb, fc_inform_rrc_for_pdcp_failure, ue);
...
/* Firecell APIs for triggering test functions*/

bool fc_manipulate_rx_deliv_count(ue_id_t ue_id, int srb_flag, int rb_id)
{
  nr_pdcp_ue_t *ue;
  nr_pdcp_entity_t *rb;
  ...
  ue = nr_pdcp_manager_get_ue(nr_pdcp_ue_manager, ue_id);
  if (!ue) {
    LOG_E(PDCP, "fc_manipulate_rx_deliv_count: UE not found\n");
    return false;
  }
  rb = nr_pdcp_get_rb(ue, rb_id, srb_flag);
  if (!rb) {
    LOG_E(PDCP, "fc_manipulate_rx_deliv_count: RB not found\n");
    return false;
  }

  /* we want to trigger below condition
  rcvd_count < entity->rx_deliv || nr_pdcp_sdu_in_list(entity->rx_list, rcvd_count)
  nr_pdcp_sdu_in_list is not a the case we want. It sdu duplication, we are discarding.
  We are interested in rcvd_count < entity->rx_deliv
  */
  LOG_W(PDCP, "Rx Deliv is increased for UE: %lu, RB: %d\n", ue_id, rb_id);
  rb->rx_deliv += 5000;
  nr_pdcp_manager_unlock(nr_pdcp_ue_manager);
  return true;
}

bool fc_trigger_integrity_failure(ue_id_t ue_id, int srb_flag, int rb_id)
{
  nr_pdcp_ue_t *ue;
  nr_pdcp_entity_t *rb;
  ...
  ue = nr_pdcp_manager_get_ue(nr_pdcp_ue_manager, ue_id);
  if (!ue) {
    LOG_E(PDCP, "fc_trigger_integrity_failure: UE not found\n");
    return false;
  }
  rb = nr_pdcp_get_rb(ue, rb_id, srb_flag);
  if (!rb) {
    LOG_E(PDCP, "fc_trigger_integrity_failure: RB not found\n");
    return false;
  }

  /* we want to fail integrity check
     for reception we will manipulate the recvd_count
  */
  LOG_W(PDCP, "Integrity failure flag is set to true for UE: %lu, RB: %d\n", ue_id, rb_id);
  rb->fc_trigger_integrity_failure = true;
  nr_pdcp_manager_unlock(nr_pdcp_ue_manager);
  return true;
}
