                                   ...
                                   long *mcs_C_RNTI)
{
  ...
  else if (!mcs_C_RNTI && mcs_Table && *mcs_Table == NR_PDSCH_Config__mcs_Table_qam64LowSE
           && ss_type == NR_SearchSpace__searchSpaceType_PR_ue_Specific && rnti_type == TYPE_C_RNTI_)
                                   ...
                                   fapi_nr_dci_indication_pdu_t *dci_ind)
{
  ...
  if (rnti_type == TYPE_C_RNTI_ && dci_ind->ss_type != NR_SearchSpace__searchSpaceType_PR_common
      ...
      for (int csi_idx = 0; csi_idx < csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.count; csi_idx++) {
        if (csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.array[csi_idx]->nzp_CSI_ResourceSetId ==
          ...
          if (mapping_type == ON_PUSCH) {
            ...
            temp_payload_2 = (mac->csirs_measurements.i1 << pmi_x2_bitlen) | mac->csirs_measurements.i2;
          } else {
