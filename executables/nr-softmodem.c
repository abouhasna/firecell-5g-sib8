...
static int create_gNB_tasks(ngran_node_t node_type, configmodule_interface_t *cfg)
{
  ...
  if (gnb_nb > 0) {
    ...
    if (NODE_IS_CU(node_type) || NODE_IS_MONOLITHIC(node_type)) {
      LOG_I(FC_COLLECTOR, "COLLECTOR: Creating CU COLLECTOR \n");
      cu_collector_config_t cu_cfg = {0};
      get_cu_collector_cfg(&cu_cfg);
      ittiTask_parms_t *args_cu_collector_p = calloc(1, sizeof(*args_cu_collector_p));
      args_cu_collector_p->args_to_start_routine = (void *)&cu_cfg;
      if (NODE_IS_CU(node_type)) {
        nr_collector_trigger = nr_collector_trigger_fake;
      }
        itti_create_task(TASK_CU_COLLECTOR, NR_CU_Collector_Task, args_cu_collector_p);
        /* TODO: firecell - hack. Explain this hack. CU listener and DU listener uses same memory address to read config args.
        if we do not put sleep, since they are seperate threads, they will try to create socket with same address
        We can put a mutex or somehow we need to understand why this memory is shared.
        */
        sleep(5);
    }

    if (NODE_IS_DU(node_type) || NODE_IS_MONOLITHIC(node_type)) {
      LOG_I(FC_COLLECTOR, "COLLECTOR: Creating DU COLLECTOR \n");
      du_collector_config_t du_cfg = {0};
      get_du_collector_cfg(&du_cfg);
      ittiTask_parms_t *args_du_collector_p = calloc(1, sizeof(*args_du_collector_p));
      args_du_collector_p->args_to_start_routine = (void *)&du_cfg;

      nr_collector_trigger = nr_collector_trigger_func;
      
      if (itti_create_task (TASK_DU_COLLECTOR, NR_DU_Collector_Task, args_du_collector_p) < 0) {
        LOG_E(FC_COLLECTOR, "Create task for NR DU Collector is failed\n");
