...
static void UE_synch(void *arg) {
  ...
  if (ret.cell_detected) {
    ...
    const int hw_slot_offset = ((ret.rx_offset << 1) / fp->samples_per_subframe * fp->slots_per_subframe)
...
void processSlotTX(void *arg)
{
    ...
    */
    ...
    if (UE->sl_mode == 2 && proc->tx_slot_type == NR_SIDELINK_SLOT) {
      ...
      if (phy_data.sl_tx_action) {
        AssertFatal((phy_data.sl_tx_action >= SL_NR_CONFIG_TYPE_TX_PSBCH && phy_data.sl_tx_action < SL_NR_CONFIG_TYPE_TX_MAXIMUM),
                    "Incorrect SL TX Action Scheduled\n");
  ...
  int slots_per_frame = (UE->sl_mode == 2) ? UE->SL_UE_PHY_PARAMS.sl_frame_params.slots_per_frame : UE->frame_parms.slots_per_frame;
